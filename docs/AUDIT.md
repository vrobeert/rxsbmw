...
- Evenimente: lista, detaliu eveniment, inscriere simpla, QR in profil si status plata gestionat de staff/admin. Lipsesc anulare inscriere, capacitate/waitlist in UI, galerie si Waze.
- Scan QR: camera, verificare manuala, validare token in Supabase, respingere bilet invalid/neplatit/deja scanat si scriere `checked_in_at`.
- Admin: dashboard, cautare membri, export CSV, lista participanti eveniment, schimbare status plata, moderare afisata. Lipsesc CRUD membri/evenimente/sponsori/anunturi, schimbare rol/nivel/cotizatie, audit log.
...
- `Admin > Vezi eveniment`: rezolvat, duce la pagina evenimentului curent.
- `Admin > Lista participanti`: rezolvat, scroll la tabelul real de inscrieri.
- `CarDetail`: afiseaza like/comment count, dar nu permite like/comentariu.
- `ScannerPage`: rezolvat pentru check-in real in Supabase.
