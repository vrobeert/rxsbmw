...
begin
  ...
  if not exists (select 1 from pg_type where typnamespace = 'public'::regnamespace and typname = 'member_level') then
...
alter table public.profiles add column if not exists member_level public.member_level not null default 'Bronze';
alter table public.profiles add column if not exists member_code text default ('BH-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 8)));
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'cars_cover_photo_fk') then
...
create table if not exists public.car_mods (
  id uuid primary key default gen_random_uuid(),
  car_id uuid not null references public.cars(id) on delete cascade,
...
set search_path = public
as $$
  select coalesce((select role in ('admin', 'staff') from public.profiles where id = auth.uid() and is_active = true), false);
$$;
...
drop policy if exists "car photos read through visible car" on public.car_photos;
...
drop policy if exists "car photos owner manage" on public.car_photos;
...
drop policy if exists "car mods read through visible car" on public.car_mods;
...
drop policy if exists "car mods owner manage" on public.car_mods;
...
drop policy if exists "comments own insert" on public.car_comments;
create policy "comments own insert" on public.car_comments
for insert with check (profile_id = auth.uid());

drop policy if exists "comments owner or staff update" on public.car_comments;
...
drop policy if exists "staff sponsors manage" on public.sponsors;
...
drop policy if exists "event children public read" on public.event_schedule_items;
...
drop policy if exists "event sponsors public read" on public.event_sponsors;
create policy "event sponsors public read" on public.event_sponsors for select using (true);

drop policy if exists "event children staff manage" on public.event_schedule_items;
...
drop policy if exists "event categories staff manage" on public.event_categories;
...
drop policy if exists "event sponsors staff manage" on public.event_sponsors;
...
drop policy if exists "registrations own or staff read" on public.event_registrations;
create policy "registrations own or staff read" on public.event_registrations
for select using (profile_id = auth.uid() or public.is_staff());

drop policy if exists "registrations own insert" on public.event_registrations;
create policy "registrations own insert" on public.event_registrations
for insert with check (profile_id = auth.uid());

drop policy if exists "registrations staff update" on public.event_registrations;
...
drop policy if exists "phase2 owner read marketplace" on public.marketplace_listings;
...
drop policy if exists "phase2 owner manage marketplace" on public.marketplace_listings;
...
drop policy if exists "phase2 service own" on public.service_entries;
...
drop policy if exists "phase2 push own" on public.push_subscriptions;
create policy "phase2 push own" on public.push_subscriptions
for all using (profile_id = auth.uid()) with check (profile_id = auth.uid());

drop policy if exists "phase2 sponsor metrics staff" on public.sponsor_metrics;
...
create index if not exists cars_profile_id_idx on public.cars(profile_id);
...
create index if not exists marketplace_listings_profile_id_idx on public.marketplace_listings(profile_id);
create index if not exists service_entries_car_id_idx on public.service_entries(car_id);
